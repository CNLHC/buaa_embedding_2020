...
static int zlg7290_hw_read(struct zlg7290 *zlg7290, int len, size_t *retlen, char *buf)
{
     ...
     */
	struct i2c_msg msg[] = { 
		{ client->addr, 0, len, buf},  // Todo：第一个i2c_msg用来发送需要读取的从设备目标寄存器的地址，buf保存待读寄存器的地址
		{ client->addr, I2C_M_RD, len, buf },  //Todo：第二个i2c_msg用来设置接收从设备目标寄存器返回数据的buf以及数据长度，buf保存从寄存器内读到的值的地址
	};
